...
export class Pawn extends Piece {
    ...
    moves(game: Game): number[][] {
        ...
        const forwardDirection =
            this.team === Team.white ? Directions.N : Directions.S;
        ...
        if (game.isInTheGrid(rightDiagonal)) {
            ...
            if (
                ...
                rightDiagonalPiece?.team !== this.team
            ) {
        ...
        if (game.isInTheGrid(leftDiagonal)) {
            ...
            if (
                ...
                leftDiagonalPiece?.team !== this.team
            ) {
    ...
    move(game: Game, tile: Tile): boolean {
        return this.canEnPassant(game)
            ? this.enPassant(game, tile)
            : super.move(game, tile);
    }
    ...
    canEnPassant(game: Game) {
        ...
        return (
            ...
            Math.abs(
                lastMove.targetTile.coords[0] - this.currentTile.coords[0]
            ) === 1
        );
    }
    ...
    clone(): Piece {
        return new Pawn(this.team, this.movesCount);
    }
}
